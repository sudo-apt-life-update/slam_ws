#!/usr/bin/env python3
# [COPIED 2026-08-19] from waypoint_follow_ws/src/g1_walk/g1_walk/loco_cli.py
# so slam_ws can run standalone. That workspace is the original and is
# left untouched; the two copies WILL drift, so check both when fixing
# anything here. Only the g1_walk -> g1_locomotion imports were changed.
...
bridge uses, so both can be connected at once -- ZMQ fair-queues the requests.
    ...
    ros2 run g1_locomotion loco_cli status
    ros2 run g1_locomotion loco_cli stand_up
    ros2 run g1_locomotion loco_cli start
    ros2 run g1_locomotion loco_cli stop
    ros2 run g1_locomotion loco_cli damp
...
longer than 10 s:
    ...
    ros2 run g1_locomotion loco_cli move --vx 0.3 --seconds 5
"""
...
from g1_locomotion.loco_protocol import (
    FSM_IDS, LocoClient, LocoError, LocoTimeout, describe_error,
)
from g1_locomotion.path import Segment
...
STUCK_HINTS = {
    0: ("zero_torque -- the robot is limp and has no holding force.\n"
        ...
        "    ros2 run g1_locomotion loco_cli damp        (take up the joints)\n"
        "    ros2 run g1_locomotion loco_cli stand_up\n"
    ...
    try:
        ...
        if status.fsm_id >= 0 and status.fsm_id != 200:
            print(f"refusing to move: fsm_id is {status.fsm_id}, not 200 "
                  ...
                  f"  Fix: ros2 run g1_locomotion loco_cli main_operation",
                  file=sys.stderr)
            return 2
...
def _wait_for_fsm(client, name, target):
    ...
    if hint:
        ...
        print(f"  The robot may still be finishing the previous transition. "
              f"Let it settle,\n  then re-send: ros2 run g1_locomotion loco_cli "
              f"{name}\n"
    ...
    try:
        ...
        if args.command == "fsm_id":
            ...
            print("continuous_gait: the robot will MARCH IN PLACE until you "
                  ...
                  "these processes.\n"
                  "  To stop:  ros2 run g1_locomotion loco_cli halt")
